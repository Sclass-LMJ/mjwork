Sub 다중파일부가세정리()
    ...
    ' FileDialog로 여러 텍스트 파일 선택
    Set fd = Application.fileDialog(msoFileDialogFilePicker)
    
    With fd
        .Title = "부가가치세 신고서 텍스트 파일을 선택하세요 (여러 개 선택 가능)"
        .Filters.Clear
    ...
    For Each filePath In selectedFiles
        If fso.FileExists(filePath) Then
            ' 단일 파일 처리
            Call ProcessSingleFile(CStr(filePath), fso, wsResult, resultRow)
            fileCount = fileCount + 1
        Else
            MsgBox "파일을 찾을 수 없습니다: " & filePath, vbExclamation
        End If
    Next filePath
    
    ' 서식 정리
    If resultRow > 2 Then ' 데이터가 있는 경우에만
    ...
    MsgBox fileCount & "개 파일의 부가세 정리가 완료되었습니다.", vbInformation
    
End Sub

' 단일 파일 처리 함수
Private Sub ProcessSingleFile(ByVal textFilePath As String, ByVal fso As Object, ByVal wsResult As Worksheet, ByRef resultRow As Long)
    ...
    ' 파일명 추출 (확장자 제외)
    fileNameOnly = fso.GetBaseName(textFilePath)
    ...
    Else
        MsgBox "필요한 페이지를 찾을 수 없습니다: " & fileNameOnly, vbExclamation
        Exit Sub
    End If
    
    ' *** 중요: 각 파일마다 "수   시   부   과   세   액   ( 2 5 )" 텍스트 존재 여부 확인 ***
    useSheet2 = (InStr(necessaryPage, "수   시   부   과   세   액   ( 2 5 )") > 0)
    
    ' 적절한 정보 시트 선택 (각 파일마다 다를 수 있음)
    If useSheet2 Then
        On Error Resume Next
        ...
        If wsInfo Is Nothing Then
            MsgBox "부가세정보시트_2를 찾을 수 없습니다. 파일 [" & fileNameOnly & "]에 대해 기본 시트를 사용합니다.", vbExclamation
            Set wsInfo = ThisWorkbook.Worksheets("부가세정보시트")
        End If
    Else
        Set wsInfo = ThisWorkbook.Worksheets("부가세정보시트")
    ...
    ' wsInfo가 제대로 설정되었는지 확인
    If wsInfo Is Nothing Then
        MsgBox "부가세정보시트를 찾을 수 없습니다: " & fileNameOnly, vbCritical
        Exit Sub
    End If
...
FileReadError:
    MsgBox "파일 읽기 오류: " & fileNameOnly & vbCrLf & Err.Description, vbCritical
    Exit Sub
